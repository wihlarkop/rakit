    ...
    def _validate_action_contract(self) -> "ActionDefinition":
        ...
        if self.requires_concurrency:
            ...
            if not self.mutating or self.transaction_policy is not TransactionPolicy.AUTO:
                raise ValueError(
                    f"Action {self.action_id!r} strong concurrency requires a mutating "
                    "RECORD action with TransactionPolicy.AUTO"
                )
...
def _action_result_is_success(result: object) -> bool:
    ...
    return isinstance(
        result,
        ActionSuccess | ActionRedirect | ActionRefresh | ActionRendered | ActionAdvancedResponse,
    )
