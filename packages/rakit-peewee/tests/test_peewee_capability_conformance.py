...
from rakit_peewee.generated import PeeweeGeneratedResourceExecutor
from rakit_peewee.plugin import PeeweePlugin
from rakit_peewee.uow import PeeweeOperationUnitOfWorkFactory

_FILE_DESCRIPTOR, _DATABASE_PATH = tempfile.mkstemp(prefix="rakit-peewee-", suffix=".sqlite3")
os.close(_FILE_DESCRIPTOR)
DATABASE = AsyncSqliteDatabase(_DATABASE_PATH, pool_size=1)
...
class PeeweePersistenceConformanceHarness:
    def __init__(self) -> None:
        ...
        assert isinstance(executor, PeeweeGeneratedResourceExecutor)
        factory = dict(builder.unit_of_work_factories)["persistence.peewee"]
        assert isinstance(factory, PeeweeOperationUnitOfWorkFactory)
    ...
    async def _reset(self) -> None:
        await DATABASE.aexecute(Widget.delete())
    ...
    async def _rows(self) -> tuple[dict[str, Any], ...]:
        rows = await DATABASE.list(Widget.select().order_by(Widget.id).dicts())
        await self._release_connection()
    ...
    async def assert_read_semantics(self) -> None:
        ...
        for name, group, score in (
            ...
            await DATABASE.aexecute(Widget.insert(name=name, group=group, score=score))
        await self._release_connection()
...
def test_peewee_conforms_to_every_advertised_v1_capability() -> None:
    async def scenario() -> None:
        async with DATABASE:
            await DATABASE.acreate_tables([Widget])
        try:
            harness = PeeweePersistenceConformanceHarness()
            harnesses = {
            ...
            result = await run_integration_conformance(
                descriptor=PEEWEE_INTEGRATION,
                harnesses=harnesses,
                specs=CANONICAL_CONFORMANCE_SPEC_REGISTRY,
            )

            assert PEEWEE_INTEGRATION.advertised_capabilities.names == (
                "persistence.read",
                "persistence.write",
