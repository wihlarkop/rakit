...
def _standard_app() -> str:
    return _text(
        ...
        def _secret_key() -> SecretValue:
            ...
            if not value:
                raise RuntimeError("Set RAKIT_SECRET_KEY before importing the standard starter app.")
...
def _new_readme(config: InitConfig) -> str:
    if config.template is StarterTemplate.MINIMAL:
        return _text(
            ...
            uv run rakit run {config.import_package}.app:admin --server {config.server.value} --reload
            ```
            '''
        )
    ...
    return _text(
        ...
        This starter owns a development SQLite database under `var/`. The bootstrap command below creates starter and Rakit auth tables for development only; it is not a production migration strategy.

        `.env.example` documents the required secret variable, but the starter does not auto-load `.env` files. Export the variable in your shell or use your application's environment loader.
...
def _existing_readme(config: InitConfig) -> str:
    if config.template is StarterTemplate.MINIMAL:
        return _text(
            ...
            This Rakit-owned module was generated additively. No host entrypoint was modified.
    ...
    return _text(
        ...
        This Rakit-owned module was generated additively. No host entrypoint, database/session configuration, root `.env`, or root `.gitignore` was modified.

        Until you deliberately replace the generated composition, this module owns a development SQLite database and uploads under `.rakit/`. Add `.rakit/` to your VCS ignore rules if appropriate for your project.

        The module reads `RAKIT_SECRET_KEY` directly from the process environment and does not auto-load `.env` files.
...
def guidance_for(config: InitConfig) -> tuple[str, ...]:
    ...
    if not config.install_dependencies:
        guidance.append("Install dependencies when ready: " + " ".join(dependency_command_for(config)))

    if config.template is StarterTemplate.STANDARD:
        guidance.extend(
            (
                'Set RAKIT_SECRET_KEY in the process environment (the generated app does not auto-load .env).',
    ...
    if config.mode is InitMode.EXISTING:
        ...
        if config.template is StarterTemplate.STANDARD:
            guidance.append(
                "The generated standard module uses isolated .rakit/ development data until you replace its persistence composition explicitly."
            )
