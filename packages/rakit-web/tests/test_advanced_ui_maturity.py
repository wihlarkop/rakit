...
class _OrdersWithActions(Admin.__mro__[1].__mro__[0]):
    """Type-only placeholder avoided at runtime; ResourceAdmin is defined below."""


from rakit_core.admin_types import ResourceAdmin  # noqa: E402
...
def test_public_action_presentation_validation_is_atomic_and_scope_aware() -> None:
    ...
    presentations = {
        str(action.definition.action_id): action_web_presentation(action.definition).intent
        for action in compiled.compiled_actions
    }
...
def _request(principal_permissions: frozenset[str]) -> Request:
...
async def test_context_action_views_hide_disable_and_authorize_entry_points(
    ...
    def make_action(
        ...
        *,
        ...
        if scope is ActionScope.PAGE:
            kwargs["page_id"] = owner_id
        else:
            kwargs["resource_id"] = owner_id
    ...
    make_action(
        "hidden",
        lambda _context: ActionAvailabilityDecision.hidden(),
    )
...
class _BulkHarness:
    ...
    def app(self) -> Starlette:
        ...
        binding = BulkActionBinding(
            ...
            record_version=(
                lambda record: cast(dict[str, object], record)["version"]
                if policy.require_concurrency_snapshot
                else None
            ),
        )
...
async def test_bulk_get_is_selection_aware_and_post_rechecks_fresh_state() -> None:
    ...
    async with httpx.AsyncClient(
        ...
        assert "Archive selected" in available.text
...
def _action_app(
    ...
    action = action.model_copy(update={"permission": permission})
    if presentation is not None:
        bind_action_web_presentation(action, presentation)
    compiled = CompiledActionDefinition(definition=action, permission=permission)
    route = RouteDefinition(
        route_name=f"resource:orders:action:{action.action_id}",
        methods=("GET", "POST"),
        path=f"/orders/_actions/{action.action_id}",
        owner_id="orders",
    )
    token_service = TokenService.single_key(
        key_id="action-ui",
