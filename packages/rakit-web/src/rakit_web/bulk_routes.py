...
from rakit_core.idempotency import IdempotencyReservation, IdempotencyStatus, IdempotencyStore, OperationReceipt
...
def _verify_confirmation(
    ...
    return claims == _confirmation_claims(
        request, action, authorization, selection, binding.codec
    )
...
def _selection_tokens_from_form(form: FormData) -> list[str]:
    return [value for value in form.getlist("selected") if isinstance(value, str) and value]
...
async def _resolve_selection(
    ...
    if len(identities) > policy.synchronous_maximum:
        raise RakitError(
            code=ErrorCode.VALIDATION_FAILED,
            message="Bulk selection exceeds the synchronous execution limit.",
            status_code=400,
...
def _parse_input(action: ActionDefinition, form: FormData) -> tuple[dict[str, object], FormState | None]:
    reserved = {
        "csrf_token",
        "submission_token",
        "confirmation_token",
...
async def _run_plan[
    ...
    deadline = Deadline.after(binding.deadline_seconds) if binding.deadline_seconds else None
    ...
    async def run() -> TResult:
        ...
        with activate_operation_context(operation_context):
            ...
            awaitable = run_operation_plan(plan, operation_context, unit_of_work_factory=uow_factory)
            return await awaitable if deadline is None else await run_with_deadline(awaitable, deadline)
...
def _completed_response(request: Request, receipt: OperationReceipt | None, fallback: str) -> Response:
    ...
    location = receipt.redirect_route if isinstance(receipt.redirect_route, str) else fallback
...
def _outcome_response(request: Request, outcome: BulkActionOutcome, fallback: str) -> Response:
    message = (
        f"Bulk action completed: {outcome.succeeded_count}/{outcome.selected_count} succeeded"
    )
...
def build_bulk_action_routes(binding: BulkActionBinding) -> list[Route]:
    ...
    for route_definition, compiled in binding.routes:
        ...
        async def bulk_get(
            ...
            try:
                selection = await _resolve_selection(
                    binding, action, _selection_tokens_from_query(request)
                )
            ...
            confirmation_token = (
                _issue_confirmation(binding, request, action, root_authorization, selection)
        ...
        async def bulk_post(
            ...
            try:
                selection = await _resolve_selection(
                    binding, action, _selection_tokens_from_form(form)
                )
            ...
            if _confirmation_required(action, len(selection.targets)):
                ...
                if not isinstance(token, str) or not _verify_confirmation(
                    ...
                    return _rejected_response(
                        request, "Bulk action confirmation is invalid or stale", 409
                    )
            try:
            ...
            fingerprint = _selection_fingerprint(action, selection, binding.codec, submitted)
            try:
                ...
                return _rejected_response(
                    request, "Submission token is bound to another bulk action", 409
                )
            ...
            if policy.execution is BulkExecutionPolicy.ATOMIC:
                ...
                try:
                    outcome = await _run_plan(
                        binding, request, plan, root_authorization
                    )
                except RakitError as exc:
                    await fail_final()
                    return _rejected_response(request, exc.message, exc.status_code)
                ...
                for context in contexts:
                    ...
                    try:
                        result = await _run_plan(
                            binding, request, plan, context.authorization
                        )
            ...
            await binding.idempotency_store.complete(
                reservation,
                _receipt(reservation, outcome, fallback_location=fallback),
            )
            return _outcome_response(request, outcome, fallback)
