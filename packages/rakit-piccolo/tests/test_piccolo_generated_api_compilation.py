...
def test_piccolo_generated_crud_compiles_with_resource_owned_uow(tmp_path: Path) -> None:
    engine = SQLiteEngine(path=str(tmp_path / "piccolo-compile.sqlite3"))

    class CompileWidget(Table, db=engine):
        name = Varchar()
    ...
    assert dict(compiled.resource_unit_of_work_provider_ids) == {"widgets": "persistence.piccolo"}
    assert "widgets" in dict(compiled.generated_resource_executor_providers)
    assert "persistence.piccolo" in dict(compiled.unit_of_work_factories)
