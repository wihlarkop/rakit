...
# Aggregate capability analysis replaces unpublished fail-fast helper.
write(
...
# Compiler configured integration registry and aggregate capability failure.
replace_once(
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            code=ErrorCode.CONFIG_INVALID,
            message=(
                "Capability requirements are not satisfied: " + ", ".join(missing_ids) + "."
            ),
            status_code=500,
            details={
...
# Public first-release core discovery contracts.
replace_once(
...
# Web + schema descriptors and configured inventory.
write(
...
# Persistence descriptor + configured inventory.
write(
...
# Auth descriptor markers preserve class docstrings.
write(
...
# Storage descriptor + configured inventory; lazy package root keeps descriptor lightweight.
write(
...
__all__ = ["STORAGE_LOCAL_INTEGRATION"]
''',
)
replace_once(
...
# Generic installed integration entry points.
for path, marker, entries in (
    (
        "packages/rakit-web/pyproject.toml",
        "[tool.uv.sources]",
...
[tool.uv.sources]''',
    ...
    (
        ...
        "[tool.uv.sources]",
        '''[project.entry-points."rakit.integrations"]
"persistence.sqlalchemy" = "rakit_sqlalchemy.discovery:SQLALCHEMY_INTEGRATION"
...
[tool.uv.sources]''',
    ...
    (
        ...
        "[tool.uv.sources]",
        '''[project.entry-points."rakit.integrations"]
"auth.sqlalchemy" = "rakit_auth_sqlalchemy.discovery:AUTH_SQLALCHEMY_INTEGRATION"
...
[tool.uv.sources]''',
    ...
    (
        ...
        "[tool.uv.sources]",
        '''[project.entry-points."rakit.integrations"]
"storage.local" = "rakit_storage_local.discovery:STORAGE_LOCAL_INTEGRATION"

[tool.uv.sources]''',
    ),
):
    replace_once(path, marker, entries)

# Server descriptors share capability constants with runtime adapters and preserve runtime entry points.
write(
...
replace_once(
    "packages/rakit-server-uvicorn/pyproject.toml",
...
replace_once(
    "packages/rakit-server-granian/pyproject.toml",
    '''[project.entry-points."rakit.servers"]
granian = "rakit_server_granian:GranianServer"
...
[project.entry-points."rakit.integrations"]
"server.granian" = "rakit_server_granian.discovery:GRANIAN_INTEGRATION"
...
# Shared human/JSON inspection model.
write(
    ...
    if report.target is not None:
        ...
        if report.configured.integrations:
            for integration in report.configured.integrations:
                ...
                lines.append(
                    f"  {identifier}: {integration.display_name} ({integration.category})"
                )
        else:
            lines.append("  none")
        lines.append("")
...
# CLI: validator remains check; inspector is capabilities.
replace_once(
...
old_block = '''def _capability_diagnostics(compiled: CompiledApplication) -> None:
...
'''
new_block = '''def _echo_check_capabilities(compiled: CompiledApplication) -> None:
...
'''
replace_once("packages/rakit/src/rakit/cli.py", old_block, new_block)
